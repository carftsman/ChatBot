 ...
 * 
 * import com.dhatvibs.modules.entities.auth.CbUser; import
 * com.dhatvibs.modules.entities.chat.*; import
 * com.dhatvibs.modules.repository.auth.CbUserRepository; import
 * com.dhatvibs.modules.repository.chat.CbOrderRepository; import
 * com.dhatvibs.modules.repository.chat.CbOrderStatusHistoryRepository;
 * 
 * import java.util.*;
 * 
 * @Service
 ...
 * userRepo;
 * 
 * // ───────────────────────────────────────────── // Get order status // Uses
 * contextOrderId if available // Falls back to latest order by userId //
 * ───────────────────────────────────────────── public String getOrderStatus(
 * String externalUserId, String appId, UUID contextOrderId) {
 * 
 * Optional<CbOrder> order;
 * 
 * if (contextOrderId != null) { // Use order from session context directly
 * order = orderRepo.findById(contextOrderId); } else { // Fallback — find
 * latest order by userId CbUser user = findUser(externalUserId, appId); if
 * (user == null) return "User not found."; order = getActiveOrder(user, appId);
 * }
 * 
 ...
 * Optional<CbOrder> order;
 * 
 * if (contextOrderId != null) { order = orderRepo.findById(contextOrderId); }
 * else { CbUser user = findUser(externalUserId, appId); if (user == null)
 * return "User not found."; order = getActiveOrder(user, appId); }
 * 
 * if (order.isEmpty()) return "No order found to show timeline.";
 * 
 * List<CbOrderStatusHistory> history = historyRepo
 * .findByCbOrderIdOrderByChangedAtAsc( order.get().getId());
 * 
 * if (history.isEmpty()) return "No status history available yet.";
 * 
 * StringBuilder sb = new StringBuilder(); sb.append("📦 Order Journey:\n");
 * history.forEach(h -> sb .append("✅ ") .append(formatStatus(h.getToStatus()))
 * .append(" — ") .append(h.getChangedAt() .toLocalTime() .toString()
 ...
 * // ───────────────────────────────────────────── public String
 * checkCancelEligibility( String externalUserId, String appId, UUID
 * contextOrderId) {
 * 
 * Optional<CbOrder> order; if (contextOrderId != null) { order =
 * orderRepo.findById(contextOrderId); } else { CbUser user =
 * findUser(externalUserId, appId); if (user == null) return "User not found.";
 * order = getActiveOrder(user, appId); }
 * 
 * if (order.isEmpty()) return "No active order found to cancel.";
 * 
 * String status = order.get().getOrderStatus(); if
 * (status.equals("ORDER_PLACED")) { return "✅ Your order can be cancelled.\n" +
 * "Please go to Orders section and " + "tap Cancel Order."; } else if
 * (status.equals("ACCEPTED")) { return "⚠️ Your order is already accepted " +
 * "by the restaurant.\n" + "Cancellation may not be possible. " +
 * "A support ticket will be raised."; } else { return
 * "❌ Your order cannot be cancelled " + "at this stage.\n" + "Status: " +
 * formatStatus(status); } }
 * 
 * // ───────────────────────────────────────────── // Get order history (last
 * 5) // ───────────────────────────────────────────── public String
 * getOrderHistory( String externalUserId, String appId) {
 * 
 * CbUser user = findUser(externalUserId, appId); if (user == null) return
 * "User not found.";
 * 
 * List<CbOrder> orders = orderRepo .findTop5ByCbUserIdOrderByPlacedAtDesc(
 * user.getId());
 ...
 * .append(formatStatus(o.getOrderStatus())) .append("\n"));
 * 
 * return sb.toString(); }
 * 
 * // ───────────────────────────────────────────── // PRIVATE HELPERS //
 * ───────────────────────────────────────────── private CbUser findUser( String
 * externalUserId, String appId) { return userRepo
 * .findByExternalUserIdAndAppId( externalUserId, appId) .orElse(null); }
 * 
 * private Optional<CbOrder> getActiveOrder( CbUser user, String appId) { return
 * switch (appId) { case "USER" -> orderRepo.findActiveOrderByUserId(
 * user.getId()); case "VENDOR" -> orderRepo.findActiveOrderByVendorId(
 * user.getId()); case "RIDER" -> orderRepo.findActiveOrderByRiderId(
 * user.getId()); default -> Optional.empty(); }; }
 * 
 * private String buildOrderStatusReply(CbOrder o) { StringBuilder sb = new
 * StringBuilder(); sb.append("🛒 Order: ")
 * .append(o.getExternalOrderId()).append("\n"); sb.append("🍽 Restaurant: ")
 * .append(o.getRestaurantName()).append("\n"); sb.append("📦 Status: ")
 * .append(formatStatus(o.getOrderStatus())) .append("\n");
 * sb.append("🛍 Items: ") .append(o.getItemsSummary()).append("\n");
 * sb.append("💰 Total: ₹") .append(o.getTotalAmount()).append("\n");
 * 
 * if (o.getEstimatedDelivery() != null &&
 * !o.getOrderStatus().contains("CANCEL") && !o.getOrderStatus()
 * .equals("DELIVERED")) { sb.append("⏱ ETA: ") .append(o.getEstimatedDelivery()
 * .toLocalTime() .toString() .substring(0, 5)) .append("\n"); }
 * 
 * if (o.getOrderStatus().contains("CANCEL")) { sb.append("❌ Reason: ")
 * .append(o.getCancelReason()).append("\n"); } return sb.toString(); }
 * 
 * private String formatStatus(String status) { return switch (status) { case
 * "ORDER_PLACED" -> "✅ Order Placed"; case "ACCEPTED" -> "✅ Accepted"; case
 * "PREPARING" -> "👨‍🍳 Preparing"; case "READY_FOR_PICKUP" ->
 * "📦 Ready for Pickup"; case "RIDER_ASSIGNED" -> "🚴 Rider Assigned"; case
 * "PICKED_UP" -> "🚴 Picked Up"; case "OUT_FOR_DELIVERY" ->
 * "🚚 Out for Delivery"; case "DELIVERED" -> "✅ Delivered"; case
 * "CANCELLED_BY_USER" -> "❌ Cancelled by You"; case "CANCELLED_BY_VENDOR" ->
 * "❌ Cancelled by Restaurant"; case "CANCELLED_BY_SYSTEM" ->
 * "❌ Cancelled by System"; default -> status; }; } }
 */  
...
public interface OrderQueryService {
    ...
    // Get order status — uses contextOrderId if available
    String getOrderStatus(
            String externalUserId,
            String appId,
            UUID contextOrderId);

    // Get order timeline step by step
    String getOrderTimeline(
            String externalUserId,
            String appId,
            UUID contextOrderId);

    // Check if order can be cancelled
    String checkCancelEligibility(
            String externalUserId,
            String appId,
            UUID contextOrderId);
