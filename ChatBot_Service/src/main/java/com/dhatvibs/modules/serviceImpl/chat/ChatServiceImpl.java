...
public class ChatServiceImpl implements ChatService {
 ...
 public WebSocketChatResponse processMessage(
     ...
     if (!Boolean.TRUE.equals(session.getChatEnabled())) {
         WebSocketChatResponse blocked =
             WebSocketChatResponse.builder()
                 .reply("Please use the FAQ options above.")
                 .sessionId(session.getId())
                 .senderType("SYSTEM")
                 .status("OPEN")
                 .timestamp(LocalDateTime.now())
                 .build();
         messagingTemplate.convertAndSend(
             "/topic/session/" + session.getId(), blocked);
         return blocked;
     }
     ...
     // ── PHASE 2: LLM replaces keyword matching ──
     ...
     if (!llmResult.needsDB()) {
         // LLM handles directly — greeting, thanks etc.
         ...
         // If DB returned nothing useful — raise ticket
 ...
 // Helper — get user name for LLM prompt
