 ...
 * 
 * @Tag(name = "Admin", description = "Admin ticket management — no auth")
 * public class AdminTicketController {
 * 
 * private final RiderTicketRepository ticketRepo;
 * 
 * @Operation(summary = "Get all rider tickets", description =
 * "Filter by status. No auth needed.")
 * 
 * @GetMapping public ResponseEntity<Page<RiderTicket>> getTickets(
 * 
 ...
 * 
 * Page<RiderTicket> tickets = status != null ? ticketRepo.findByStatus( status,
 ...
 * 
 * @GetMapping("/{ticketId}") public ResponseEntity<RiderTicket> getTicket(
 * 
 * @PathVariable UUID ticketId) { return ResponseEntity.ok(
 * ticketRepo.findById(ticketId) .orElseThrow(() -> new RuntimeException(
 * "Ticket not found"))); }
 * 
 * @Operation(summary = "Update ticket status", description =
 * "Assign or resolve ticket")
 * 
 * @PutMapping("/{ticketId}/status") public ResponseEntity<RiderTicket> update(
 * 
 * @PathVariable UUID ticketId,
 * 
 * @RequestBody TicketUpdateRequest req) {
 * 
 * RiderTicket ticket = ticketRepo .findById(ticketId) .orElseThrow(() -> new
 ...
 * ticketRepo.countByStatus("OPEN"), "inProgress",
 * ticketRepo.countByStatus("IN_PROGRESS"), "resolved",
 * ticketRepo.countByStatus("RESOLVED"))); } }
 */


@RestController
@RequestMapping("/rider/admin/tickets")
@RequiredArgsConstructor
public class AdminTicketController {

    // MUST be RiderTicketRepository
    private final RiderTicketRepository ticketRepo;

    @GetMapping
    public ResponseEntity<Page<RiderTicket>> getTickets(
            @RequestParam(required = false) String status,
            @RequestParam(defaultValue = "0") int page,
        ...
        Page<RiderTicket> tickets = status != null
